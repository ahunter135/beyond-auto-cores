...
namespace Onsharp.BeyondAutoCore.Web.ServiceClient
{
    public abstract class ApiClient
    {
        ...
        protected ApiClient(string host, int port, bool enableSSL, string token)
        {
            _host = host;
            _port = port;
        ...
        private async Task<HttpResponseMessage> GetByIdRequest(object id)
        {
            ...
            try
            {
                client = new HttpClient();
                string uri = string.Format("{0}://{1}:{2}/{3}/{4}/{5}",
                              _scheme, _host, _port, _root, Service, id);
                request = new HttpRequestMessage(HttpMethod.Get, uri);
                AddAuthHeader(request);
                response = await Send(client, request);//.ConfigureAwait(false);
            }
        ...
        private async Task<HttpResponseMessage> PutRequest(object id, object payload)
        {
            HttpResponseMessage response = null;
        ...
        private async Task<HttpResponseMessage> PostRequest(object payload, string action = null, double timeOut = 0, bool? continueOnCapturedContext = null)
        {
            HttpResponseMessage response = null;
            using (HttpClient client = new HttpClient())
            {
                ...
                string uri = string.Format("{0}://{1}:{2}/{3}/{4}",
                      _scheme, _host, _port, _root, Service);
        ...
        private async Task<HttpResponseMessage> DeleteRequest(long id, string action = null)
        {
            ...
            {
                string uri = string.Format("{0}://{1}:{2}/{3}/{4}/{5}",
                               _scheme, _host, _port, _root, Service, string.IsNullOrWhiteSpace(action) ? id.ToString() : string.Format("details/", id));
        ...
        private async Task<HttpResponseMessage> DeleteRequest(int id, string action = null)
        {
            ...
            {
                string uri = string.Format("{0}://{1}:{2}/{3}/{4}/{5}",
                              _scheme, _host, _port, _root, Service, string.IsNullOrWhiteSpace(action) ? id.ToString() : string.Format("{0}/{1}", action, id));
                using (var request = new HttpRequestMessage(HttpMethod.Delete, uri))
                {
                    AddAuthHeader(request);
                    response = await Send(client, request);//.ConfigureAwait(false);
...
#pragma warning restore 4014
        ...
        protected async Task<Result<T>> GetById<T>(object id)
        {
            ...
            HttpResponseMessage response = await GetByIdRequest(id);
        ...
        protected async Task<Result<T>> Get<T>(string action, Dictionary<string, string> apiParameters = null)
        {
            HttpResponseMessage response = null;
            Stream stream = null;
            StreamReader streamreader = null;
        ...
        protected async Task<Result<T>> Delete<T>(long id)
        {
            HttpResponseMessage response = null;
            Stream stream = null;
            StreamReader streamreader = null;
